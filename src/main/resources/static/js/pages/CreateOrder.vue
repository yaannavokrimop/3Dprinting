            ...
            >
                <v-form ref="form" v-model="valid" lazy-validation>
                    ...
                    <v-text-field outlined type="number" placeholder="Сумма заказа*" v-model="sum" dense
                                  :rules="[v => v<=999999 && v>0 || 'Нужно ввести число от 1 до 999999']"
                                  required></v-text-field>

                    <v-text-field outlined type="text" placeholder="Название заказа*" v-model="name" dense
                    ...
                    <v-text-field outlined type="number" placeholder="Высота изделия (в мм)*" v-model="height" dense
                                  :rules="[v => v<=2000 && v>0 || 'Нужно ввести число от 1 до 2000']"></v-text-field>
                    ...
                    <v-btn v-on:click="addOrder" variant="primary" :disabled="!valid">Добавить</v-btn>
                    <div class="mt-2"></div>
                    <v-btn v-on:click="addDraft" variant="primary" :disabled="false">Сохранить как черновик</v-btn>
                    <div class="mt-2"></div>
                </v-form>
            </b-card>
        </div>
        ...
        methods: {
                    ...
                    AXIOS.post('order', newOrder)
                        .then(response => {
                            ...
                            if (this.file) {
                                ...
                                AXIOS.patch('order/file/' + response.data.id, formData)
                                    .then(response => {
                                        console.log(response);
            ...
            addDraft() {
                ...
                if (newOrder.sum != '' || newOrder.height != '' || newOrder.width != '' ||
                    newOrder.name != '' || newOrder.length != '' || newOrder.description != '' ||
                    newOrder.materials != null || newOrder.file != null) {
                    if (this.file) {
                        this.fileName = this.file.name;
                    }
                    AXIOS.post('/order/draft', newOrder)
                        .then(response => {
                            ...
                            if (this.file) {
                                let formData = new FormData();
                                formData.append("file", this.file);
                                AXIOS.patch('/order/draft/file/' + response.data.id, formData)
                                    .then(response => {
                                        console.log(response);
                                        this.$router.push('/orders');
                                        location.reload();
